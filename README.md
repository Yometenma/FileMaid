...
当前正式版本：**1.0.0** · [查看变更记录](CHANGELOG.md)
...
升级和忘记密码的处理见 [升级与恢复](docs/upgrade-and-recovery.md)。
...
>
> GitHub 源码与发布镜像**不包含任何 API 密钥**。上表中的密钥变量默认均为空，仅供部署者在自己的 `.env` 中注入；也可登录后在设置页填写。不要提交 `.env` 或 `config/`。
