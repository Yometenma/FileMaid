...
public final class RegexMediaNameParser implements MediaNameParser {
    ...
    public ParsedMediaName parse(String fileName) {
        ...
        Matcher wordMatcher = SEASON_EPISODE_WORDS.matcher(baseName);
        if (wordMatcher.find()) {
            int season = Integer.parseInt(wordMatcher.group(1));
            List<Integer> episodes = parseEpisodes(wordMatcher.group(2));
            return result(fileName, cleanTitle(baseName.substring(0, wordMatcher.start())), MediaType.EPISODE, findYear(baseName), season, episodes, extension, 0.93);
        }
