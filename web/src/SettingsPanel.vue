...
const categories = [
  ...
  ['scan','扫描','控制扫描范围、文件数量和识别规则。'], ['system','系统','管理时区和日志级别。'], ['notification','通知','配置整理完成通知（Webhook）。']
...
const testingNotification=ref(false), notificationResult=ref('')
...
function fieldLabel(field:Definition) {
  ...
  const suffix:Record<string,string> = { enabled:'启用',endpoint:'自定义 API 地址',apiKey:'API Key',pin:'PIN' }
  return `${match[1].toUpperCase()} · ${suffix[match[2]] || match[2]}`
}
function changeCategory(id:string) { activeCategory.value=id; notice.value=''; if(id==='system')loadDiagnostics(); if(id==='notification')notificationResult.value='' }
