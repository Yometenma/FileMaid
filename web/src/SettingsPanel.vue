...
const categories = [
  ...
  ['scan','扫描','控制扫描范围、文件数量和识别规则。'], ['system','系统','管理时区、日志级别和数据库备份。']
...
const labels:Record<string,string> = {
  ...
  'files.defaultOperation':'默认文件操作','files.conflictPolicy':'冲突策略','files.historyRetentionDays':'历史保留天数','scan.maxDepth':'最大扫描深度','scan.maxFiles':'最大文件数量','scan.ignorePatterns':'忽略规则','scan.minimumFileSizeMb':'最小文件大小（MB）','scan.extensions':'扩展名白名单','system.timezone':'时区','system.logLevel':'日志级别','system.databaseBackupRetention':'备份保留数量'
}
...
function changeCategory(id:string) { activeCategory.value=id; notice.value=''; if(id==='system')loadDiagnostics() }
